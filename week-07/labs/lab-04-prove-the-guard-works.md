...
| --- | --- | --- |
| Grid Beacon `10.60.6.4` | Allow | On the allow list |
| Other Test Source `10.60.6.10` | Denied | 10.60.6.4 is not on the allow list |
...
```text
Allow is accurate to my prediction.
```
...
```text
Denied is accurate to my prediction.
```
...
| Which rule produces the intended Allow? | Priority 300 (allow-web-800) |
...
![Final rule — week07-lab04-final-rule.png](https://raw.githubusercontent.com/JocelynsJackson/Jocelyn-Jackson---Cyberfoundations-Portfolio/refs/heads/main/assets/screenshots/week-07/lab04-final-rule.png)

![Grid Beacon ALLOWED result — week07-lab04-grid-beacon-allowed.png](https://raw.githubusercontent.com/JocelynsJackson/Jocelyn-Jackson---Cyberfoundations-Portfolio/refs/heads/main/assets/screenshots/week-07/lab04-grid-beacon-allowed.png)

![Other Test Source DENIED result — week07-lab04-other-source-denied.png](https://raw.githubusercontent.com/JocelynsJackson/Jocelyn-Jackson---Cyberfoundations-Portfolio/refs/heads/main/assets/screenshots/week-07/lab04-other-source-denied.png)
...
```text
I configured a student allow rule for inbound TCP traffic on port 8080 from the Grid Beacon at 10.60.6.4. The service was listening on TCP port 8080, allowing the connection to be tested. The Grid Beacon test returned an allow result, showing that the source could reach the service. The other test source at 10.60.6.10 returned denied, showing that sources outside the allowed address could not access the service. The priority 300 allow rule allowed the Grid Beacon traffic, while the protected priority 1000 Deny rule blocked the other source. These results demonstrate least privilege because access was limited to only the required allowed source rather than allowing all sources.
```
...
```text
One allowed result and one denied result provide stronger evidence together because they show that the rules can both allow and block traffic. The allowed result proves that the connection can successfully reach the service when the rule allows it. The denied result proves that the rule can block traffic when there is no matching allow rule. Having both results helps confirm that the rules are working as expected and that the test is not just showing one outcome.
```
...
```text
I would first start by inspecting the current rule list to make sure everything is prioritized correctly. I would look for any allow and deny rules matching the other test source that are prioritized above the 1000 Deny rule. I would also double check the test traffic details source IP, destination port, direction, and protocol to confirm they match as they should. Finally, I would review the test results and the rule base to understand which rule is allowing the traffic through before making any changes.
```

**Analysis Question 3.** How does this evidence distinguish configuration from observed enforcement? (Minimum 3 sentences.)

```text
The configuration shows what rules were created and how they are intended to work, but the test results show what actually happened. The allowed and denied results provide evidence that the rules are working as expected. Comparing the configuration with the test results helps confirm that the network aligns with the intended configuration.
```
