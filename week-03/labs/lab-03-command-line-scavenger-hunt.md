...
```
PS /home/archivist> Get-Help ls
```

What the help text (or the folder's contents) told you:

```
The help text described ls is a list where the location of an item (file, folder) is in a directory
```
...
```
While working in the simulator, there were many times I had to use cd. I  consistently ended up in the wrong folders and files.
```
...
```
The Shift Log - Foundry District Archive Room was the hardest for me to find. Not sure why, but I was able to find maintenance and supply first, then returned to step one to find the Shift Log. Another point where I got 
...
```
At the start of the lesson, I felt pretty comfortable. Around the time of moving the shift log, maintenance note, and supply inventory files, I got pretty confused and was unable to get back on track with moving the logs, notes, and files. Not being able to get back on track after multiple tries left me feeling a bit defeated.
```
...
```
I am looking forward to getting more comfortable with the commands and navigating the command requests until they become second nature, and I'm no longer lost in the system.
```
