...
**[TERMINAL] Run:** test ! -e ~/.ssh/week8_analyst_ed25519 &&   test ! -e ~/.ssh/week8_analyst_ed25519.pub &&   echo "READY: Week 8 key path is unused" ||   echo "STOP: Week 8 key already exists"
...
```text
The public key can be distributed because it is meant to be shared. The private key should be kept secret because it is used to prove a user's identity and, in some file encryptions, to decrypt information.
```
...
**[TERMINAL] Run:** mkdir -p ~/.ssh and chmod 700 ~/.ssh
...
```text
The public key file (.pub) can be shared, but the private key file should be kept secure and never shared. The private key is used to prove your identity when connecting to the system. The key passphrase protects the private key by requiring a password before it can be used.
```
...
```text
The public key is meant to be shared and does not reveal the private key. The private key must remain protected because it is used to prove your identity.
```

2. How is the key passphrase different from the `analyst` account password?

```text
The key passphrase protects the private SSH key, while the analyst account password is used to authenticate the user account. The passphrase unlocks the private key so it can be used.
```

3. Why did you use a unique Week 8 filename?

```text
A unique Week 8 filename helps easily identify the file.
```
