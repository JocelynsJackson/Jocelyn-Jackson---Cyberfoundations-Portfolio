...
**Student Name:** Jocelyn Jacksonss
...
```
The Bastion/browser connection is a bridge between my computer and the Cloud Heights VM. It handled the network connection for me, allowing me to access the remote machine through my browser without me setting up the network hop.
```
...
```
Secure Shell(SSH) is a command that helps securely connect you to a remote device. 
```
...
ED25519 key fingerprint is SHA256:xkcUrbFQCt1/IXinVqb44XyyYBA+goD6HIyifWeOjBU.
...
```
I answered yes because this was my first time connecting to this local machine. I knew the connection was expected, so the fingerprint was not suspicious.
```
...
```
The prompt changed because ssh connected to a different machine. The hostname in the prompt changed to identify the remote VM, while the username and format remained the same.
```
...
![SSH first connection and nested session](https://raw.githubusercontent.com/JocelynsJackson/Jocelyn-Jackson---Cyberfoundations-Portfolio/refs/heads/main/assets/screenshots/week-06/ssh-first-connection.png)
...
```
Once the exit command is run, logout appears, and the VM shows a disconnected prompt. The prompt returns to the one from the original session, and I know I am back because the prompt shows the name of the original machine.
```
...
```
The network connection worked because SSH reached the server and prompted for a password. SSH also checked the provided login information before showing “Permission denied.” This means the problem was with authentication, not the network, because the network was responsive to commands.
```
...
```
The difference between reach and authentication is that reach means the SSH connection is working and has successfully reached the server. Authentication is the process of proving to the system that you are who you say you are. The SSH connection must succeed before the authentication process can begin.
```
...
```
Accepting a fingerprint without thinking could be a real problem if we are connecting to the wrong device. This could mean connecting to a malicious device and potentially exposing the network and sensitive information. An analogy that helped me understand fingerprints is checking someone’s badge before allowing them to enter a secure space, such as a server room. Allowing someone in without confirming their badge could mean accidentally allowing an attacker into the secure area.
```
...
```
Moving from the outer session to the nested session, the connection to the first VM stayed the same. The nested SSH session created a new connection to another machine.
```
...
```
Using what we learned in the lab, I would say that SSH is not broken because the connection is reaching the SSH service and prompting for credentials. The “permission denied” message shows that the issue is more likely related to authentication than to SSH. I would tell the user to check their username and password and to ensure they are using the correct credentials.
```
