...
morgan@foundry:/home/morgan/archive$ cp inv-march*
...
```
morgan@foundry:/home/morgan/archive$ grep -i denied door-access.log
```
...
```
02:47 DENIED badge 4471 storeroom door
This badge attempt is at 2:47 AM which is outside of normal business hours.
```
...
```
-rw-rw-rw- 1 morgan foundry   129 meeting-recap.txt
Owner, Group, and Other all have read and write access.
```
...
```
CMD:
morgan@foundry:/home/morgan/archive$ chmod go-rw meeting-recap.txt
morgan@foundry:/home/morgan/archive$ ls -l
```
...
```
ls verifies what files and folders are available before using cp. Using cp before ls can cause unwanted files and folders to be copied. When rm is attached to a command, stakes are higher because this can cause items to be permanently deleted and unable to be retrieved.
```
...
```
In real time incident when there is a physical or cybersecurity breach, it is helpful to limit the six hundred lines of information to 3 to get important information quickly. Separating standard day to ay activiy from suspicious activity helps to investigate and respond to an incident promptly.
```
...
```
Finding files by their contents is an important skill because it allows for faster discovery instead of manually searching through each folder one by one. Sometimes sensitive content can be stored in the wrong folder or mislabeled, making it harder to find through normal file organization. Auditing permissions evaluates who has access to a file and what level of access they have. Both skills are important on their own, but they are more valuable when used together because they help identify both sensitive information and potential access risks.
```
...
```
I believe ls and ls-l are important because it allows you to verify the information you are working with before taking any action. As well as the permissions for each file. It is always important to confirm the information before performing actions such as copying, removing, or moving files, as mentioned in the previous question.
```
...
![Archive investigation — find, check, lock down](https://raw.githubusercontent.com/JocelynsJackson/Jocelyn-Jackson---Cyberfoundations-Portfolio/refs/heads/main/assets/screenshots/week-04/cli-search-investigation.png)
