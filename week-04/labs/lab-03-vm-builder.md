...
```
Region: Foundry Central
I chose this region because of its location, low latency, and time for commands to reach the VM.
```
...
I feel more comfortable navigating Bash commands, so I chose this option because it is free and avoids the additional licensing costs associated with Windows.
```
...
Small workloads — the size this course's labs assume.
```
...
In this snapshot, I captured the VM's current state. Having a snapshot would be handy for auditing or troubleshooting because it provides a timestamp and restore point.
```
...
```
A locker fee is the fee that covers the storage space taken up on the server. Deletion is the only true zero because it deletes the stored data and removes the VM from the physical server.
```
...
```
A golden snapshot is a template of a VM that is already set up. When the instructor uses that snapshot to make each student's machine, everyone gets the same setup, software, and settings. This means we all start from the same place instead of having to set up our machines differently.
```
...
```
I didn't hit any provisioning errors while in the simulator. I believe this lab wants us to encounter errors in the simulator so we are able to troubleshoot in an isolated environment. before entering into the real infrastructure, where we are likely to encounter errors.
```
...
I chose the B2s size at $0.046/hr because it provided enough resources for the workload needed for these labs while keeping costs low. B1s, while cheaper, would not provide the CPU, RAM, and storage needed for the workload.

D2s_v3 and D4s_v3 provided more resources than what is needed for the workload at the moment. If the scope of the job were to change and require more resources, at that time, the VM size can be reevaluated and upgraded to meet the required needs.
```
...
**1. This worksheet, via the Lab Portal:** VM concepts + CLI screenshots
...
![VM configuration summary](https://raw.githubusercontent.com/JocelynsJackson/Jocelyn-Jackson---Cyberfoundations-Portfolio/refs/heads/main/assets/screenshots/week-04/vm-config-summary.png)

![VM dashboard — running, with snapshot](https://raw.githubusercontent.com/JocelynsJackson/Jocelyn-Jackson---Cyberfoundations-Portfolio/refs/heads/main/assets/screenshots/week-04/vm-dashboard-running.png)
