...
**RAM currently in use:** 6.3 GB

**Total installed RAM (from Part A):** 8 GB
...
```
HP System Event Utility is a pre installed Windows software that manages specials function keys and hotkey. This process works with both the hardware and OS to perform task in this example a keyboard. Certain keys are pressed triggering a specific result from such as the Ctrl+Shift+Esc opens up Task manager or Ctrl+Alt+Delete allowing us to lock our computers, switch users and sign out.The hardware is used to press the keys and the OS communicates to request. 
```
...
```
CPU usage is important to monitor in cybersecurity because it can show a trend in the activity. While it may be normal to see constant small changes. Drastic changes and sudden spikes can be indicators of malicious activity. If a unknown process has appeared and is consuming a large amount of CPU this is a process that should be investigated to determine if the system has been compromised. While high CPU alone doesn't mean the system has been compromised it is a starting point for an instigation. 
```
...
```
Watching Task Manager/Activity Monitor in real time, we are able to see the CPU, memory, RAM etc that allows us to better understand what is running on the device and create a story based on the observed trends. The static diagram is a general outline of a computers ecosystem and how each layer interact with each other. Having access to live data provides more insight that a static diagram.
```
